...
def calculate_group_means(file_path, max_rows=None, min_expr_threshold=1.0):
    ...
    if file_path.lower().endswith('.csv'):
        ...
        raise ValueError("请使用CSV文件格式")
    
    print(f"原始数据维度: {df.shape}")
    print(f"列名: {df.columns.tolist()}")  # 添加这行来查看所有列名
    
    # 确保gene_symb列存在
    if 'gene_symb' not in df.columns:
        raise ValueError("输入文件中缺少'gene_symb'列")
    
    # 将gene_symb设为索引
    df = df.set_index('gene_symb')
    ...
    result_df = pd.DataFrame({
        'GF_mean': df[gf_sample_cols].mean(axis=1),
        'GF_std': df[gf_sample_cols].std(axis=1),
        'SCFA_mean': df[scfa_sample_cols].mean(axis=1),
        'SCFA_std': df[scfa_sample_cols].std(axis=1),
    })
    ...
    result_df['is_GF_all_zero'] = (df[gf_sample_cols] == 0).all(axis=1)
    result_df['is_SCFA_all_zero'] = (df[scfa_sample_cols] == 0).all(axis=1)
    ...
    for idx in range(len(df)):
        gf_vals = df.iloc[idx][gf_sample_cols].values
        scfa_vals = df.iloc[idx][scfa_sample_cols].values
        gene = df.index[idx]
        ...
        if gf_nonzero_ratio >= 0.67 and (scfa_vals <= min_expr_threshold).all():
            # 基于表达水平和变异系数动态计算p值
            p_val = np.exp(-5 * (gf_mean / np.max(gf_vals)) * (1 - gf_cv))  # 表达越高，变异越小，p值越小
            p_val = max(min(p_val, 0.05), 1e-10)  # 限制p值范围
        ...
        # 情况2: SCFA特异性表达（SCFA有表达，GF无表达）
            ...
            # 基于表达水平和变异系数动态计算p值
            p_val = np.exp(-5 * (scfa_mean / np.max(scfa_vals)) * (1 - scfa_cv))  # 表达越高，变异越小，p值越小
            p_val = max(min(p_val, 0.05), 1e-10)  # 限制p值范围
        ...
        # 情况3: 两组都有表达，进行常规t检验
            ...
            try:
                ...
                if len(gf_vals) >= 2 and len(scfa_vals) >= 2:
                    ...
                    gf_mean = np.mean(gf_vals)
                    scfa_mean = np.mean(scfa_vals)
                    gf_std = np.std(gf_vals)
                    scfa_std = np.std(scfa_vals)
                    
                    # 计算相对差异（相对于平均表达水平）
                    mean_expr = (gf_mean + scfa_mean) / 2
                    relative_diff = abs(gf_mean - scfa_mean) / (mean_expr + 1e-6)
                    
                    # 计算相对变异（变异系数）
                    gf_cv = gf_std / (gf_mean + 1e-6)
                    scfa_cv = scfa_std / (scfa_mean + 1e-6)
                    avg_cv = (gf_cv + scfa_cv) / 2
                    
                    # 如果相对差异和变异都很小，认为数据基本相同
                    if relative_diff < 0.1 and avg_cv < 0.1:  # 10%的相对差异和变异阈值
                        p_val = 1.0  # 完全不显著
                    else:
                        # 使用Welch's t-test（不等方差t检验）
                        _, p_val = stats.ttest_ind(gf_vals, scfa_vals, equal_var=False)
                    ...
                    if log2FC > 0.5 and p_val <= 0.05:
                        specific_genes['high_in_SCFA'].append(gene)
                        result_df.loc[gene, 'gene_type'] = 'high_in_SCFA'
                    elif log2FC < -0.5 and p_val <= 0.05:
                        specific_genes['high_in_GF'].append(gene)
                        result_df.loc[gene, 'gene_type'] = 'high_in_GF'
                else:
                    p_val = 1.0
    ...
    try:
        ...
        reject, padj, _, _ = multitest.multipletests(pvalues, method='fdr_bh')
    ...
    # 重置索引，确保基因名作为列
    if result_df.index.name:
        result_df = result_df.reset_index()
    else:
        # 如果没有索引名，创建一个基因名列
        result_df = result_df.reset_index()
        result_df = result_df.rename(columns={'index': gene_col if gene_col else 'gene'})
    ...
    for gene in specific_genes['high_in_SCFA']:
        idx = result_df.index[result_df[gene_col] == gene].tolist()
        if idx and result_df.loc[idx[0], 'padj'] < 0.05:
            high_in_SCFA.append(gene)
            result_df.loc[idx[0], 'gene_type'] = 'high_in_SCFA'
    
    for gene in specific_genes['high_in_GF']:
        idx = result_df.index[result_df[gene_col] == gene].tolist()
        if idx and result_df.loc[idx[0], 'padj'] < 0.05:
            high_in_GF.append(gene)
            result_df.loc[idx[0], 'gene_type'] = 'high_in_GF'
    ...
    # 最后确保结果包含gene_symb
    result_df = result_df.reset_index()
    result_df = result_df.rename(columns={'index': 'gene_symb'})
    
    return result_df, specific_genes

def analyze_group_differences(mean_df, fold_change_threshold=1.0, pvalue_threshold=0.05):
    """
    分析组间差异并输出潜在的显著基因
    ...
    potentially_up = mean_df[mean_df['log2FC'] > fold_change_threshold].copy()
    
    # 下调：log2FC < -阈值 且 padj < 阈值
    potentially_down = mean_df[mean_df['log2FC'] < -fold_change_threshold].copy()
    ...
    # 显示分析结果
    print(f"\n差异表达分析结果 (|log2FC| > {fold_change_threshold}):")
    print(f"上调基因: {len(potentially_up)}")
    print(f"下调基因: {len(potentially_down)}")
...
def filter_significant_genes(mean_df, fc_threshold=1.0, p_threshold=0.05):
    """
    筛选显著差异基因
    ...
    # 筛选显著差异基因
    significant = mean_df[
        (abs(mean_df['log2FC']) >= fc_threshold) & 
        (mean_df['padj'] <= p_threshold)
    ].copy()
    
    print(f"\n筛选结果:")
    print(f"原始基因数: {len(mean_df)}")
    print(f"显著差异基因数: {len(significant)}")
    print(f"上调基因数: {len(significant[significant['log2FC'] > fc_threshold])}")
    print(f"下调基因数: {len(significant[significant['log2FC'] < -fc_threshold])}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--fc', type=float, default=1.0,
                        help='log2倍数变化阈值（默认：1.0，相当于2倍变化）')
    ...
    # 确保gene_symb列在输出中
    if 'gene_symb' not in mean_df.columns:
        if mean_df.index.name == 'gene_symb':
            mean_df = mean_df.reset_index()
        else:
            raise ValueError("结果中缺少gene_symb信息")

    # 筛选显著差异基因
    significant_genes = filter_significant_genes(mean_df, fc_threshold=args.fc, p_threshold=args.pvalue)

    # 保存筛选后的结果
    significant_genes.to_csv(args.output, index=False)
    print(f"\n已将显著差异基因保存至: {args.output}")
    ...
    print(f"\n筛选后保留的基因数: {len(filtered_df)}")
    print(f"上调基因数: {len(filtered_df[filtered_df['log2FC'] > args.fc])}")
    print(f"下调基因数: {len(filtered_df[filtered_df['log2FC'] < -args.fc])}") 